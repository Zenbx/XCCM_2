...
import { X, Clock, User, ChevronRight, RotateCcw } from "lucide-react";
...
function DiffView({ before, after }: { before: string; after: string }) {
    ...
    if (tokens.every((t) => t.type === "equal")) {
        return <p className="text-xs text-gray-400 italic">Aucun changement de texte détectable.</p>;
    }

    return (
        <div className="text-xs leading-relaxed font-mono bg-gray-950 rounded-lg p-3 max-h-64 overflow-y-auto">
            {tokens.map((tok, i) => {
                if (tok.type === "equal")
                    return <span key={i} className="text-gray-300">{tok.text} </span>;
                if (tok.type === "added")
                    return <span key={i} className="bg-green-900/60 text-green-300 rounded px-0.5">{tok.text} </span>;
                return <span key={i} className="bg-red-900/60 text-red-300 line-through rounded px-0.5">{tok.text} </span>;
            })}
        </div>
    );
}
...
export function NotionHistoryPanel({
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-[#1a1a2e] text-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden border border-white/10">
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-400 mt-0.5">{notionName}</p>
                        </div>
                    </div>
                        ...
                        <aside className="w-64 shrink-0 border-r border-white/10 overflow-y-auto">
                            {revisions.map((rev) => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 flex items-start gap-3 hover:bg-white/5 transition-colors border-b border-white/5 ${selected?.id === rev.id ? "bg-white/10" : ""}`}
                                >
                            ...
                            <main className="flex-1 overflow-y-auto p-5 space-y-5">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    {onRestore && (
                                        <button
                                            ...
                                            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#99334C]/20 hover:bg-[#99334C]/40 text-[#f08] rounded-lg text-xs font-medium transition-colors"
                                        >
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div
                                            className="text-xs text-gray-300 bg-red-950/30 border border-red-900/30 rounded-lg p-3 max-h-48 overflow-y-auto prose prose-invert prose-xs"
                                    ...
                                    <div>
                                        ...
                                        <div
                                            className="text-xs text-gray-300 bg-green-950/30 border border-green-900/30 rounded-lg p-3 max-h-48 overflow-y-auto prose prose-invert prose-xs"
